...
class TenantComponent(BaseModel):
    """
    TenantComponent
    """ # noqa: E501
    provider: StrictStr
    ...
    def provider_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['mtmai.tenant.Tenant']):
            raise ValueError("must be one of enum values ('mtmai.tenant.Tenant')")
        return value
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            ...
            "label": obj.get("label"),
            "config": TenantComponentConfig.from_dict(obj["config"]) if obj.get("config") is not None else None
        })
        return _obj

    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        for _key in obj.keys():
            if _key not in cls.__properties:
                raise ValueError("Error due to additional fields (not defined in AlbumList) in the input: " + _key)

        _obj = cls.model_validate({
            "pagination": PaginationResponse.from_dict(obj["pagination"]) if obj.get("pagination") is not None else None,
            "rows": [Album.from_dict(_item) for _item in obj["rows"]] if obj.get("rows") is not None else None
        })
        return _obj

...
class TenantAgent(RoutedAgent):
  ...
  @message_handler
  async def handle_tenant_init(self, message: TenantInitInput, ctx: MessageContext) -> None:
    """用户输入"""
    ...
    tid = get_tenant_id()
    from mtmai.mtlibs.autogen_utils.gallery_builder import create_default_gallery_builder
